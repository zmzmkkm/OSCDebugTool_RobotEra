...
namespace XCharts.Editor
{
    ...
    public class BasePropertyDrawer : PropertyDrawer
    {
        ...
        /// <param name="relativePropName">反射的变量</param>
        /// <param name="compareName">比较变量</param>
        protected void PropertyField(SerializedProperty prop, string relativePropName,string compareName)
        {
            if (IngorePropertys.Contains(relativePropName)) return;
            if (!ChartEditorHelper.PropertyField(ref m_DrawRect, m_Heights, m_KeyName, prop, relativePropName,compareName))
            {
                Debug.LogError("PropertyField ERROR:" + prop.displayName + ", " + relativePropName + ",CompareName:" + compareName);
            }
        }
