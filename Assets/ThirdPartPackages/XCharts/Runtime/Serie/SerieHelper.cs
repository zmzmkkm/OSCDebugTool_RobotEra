...
namespace XCharts.Runtime
{
    public static partial class SerieHelper
    {
        ...
        public static bool GetAreaColor(out Color32 color, out Color32 toColor,out Color32 shadowColor,out Color32 shadowToColor,out bool innerFill,
            out bool toTop,out bool isNormalArea,out bool isOpenCoeifficient,out float areaDisCoefficient, out float coefficient,out bool isOpenShadow,out float shadowDisCoeifficient, out float shadowCoeifficient ,Serie serie, SerieData serieData, ThemeStyle theme, int index)
        {
            color = ChartConst.clearColor32;
            toColor = ChartConst.clearColor32;
            shadowColor = ChartConst.clearColor32;
            shadowToColor = ChartConst.clearColor32;
            innerFill = false;
            toTop = true;
            isNormalArea = true;
            isOpenCoeifficient = true;
            areaDisCoefficient = 1f;
            coefficient = 1f;
            isOpenShadow = true;
            shadowDisCoeifficient = 1f;
            shadowCoeifficient = 1f;
            var state = GetSerieState(serie, serieData);
            var stateStyle = GetStateStyle(serie, serieData, state);
            if (stateStyle == null)
            {
                ...
                innerFill = areaStyle.innerFill;
                toTop = areaStyle.toTop;
                isNormalArea = areaStyle.OpenNormalArea;
                isOpenCoeifficient = areaStyle.OpenAreaCoefficient;
                areaDisCoefficient = areaStyle.AreaDisCoefficient;
                coefficient = areaStyle.AreaCoefficient;
                isOpenShadow = areaStyle.OpenAreaShadow;
                shadowDisCoeifficient = areaStyle.AreaShadowDisCoefficient;
                shadowCoeifficient = areaStyle.AreaShadowCoefficient;
                GetColor(ref color, areaStyle.color, serie.itemStyle.color, areaStyle.opacity, theme, index);
                GetColor(ref toColor, areaStyle.toColor, color, areaStyle.opacity, theme, index);
                GetColor(ref shadowColor, areaStyle.AreaShadowColor, serie.itemStyle.color, areaStyle.AreaShadowOpacity, theme, index);
                GetColor(ref shadowToColor, areaStyle.AreaShadowToColor, color, areaStyle.AreaShadowOpacity, theme, index);
                switch (state)
                {
                    case SerieState.Emphasis:
                        color = ChartHelper.GetHighlightColor(color);
            ...
            else
            {
                if (stateStyle.areaStyle.show)
                {
                    innerFill = stateStyle.areaStyle.innerFill;
                    toTop = stateStyle.areaStyle.toTop;
                    isNormalArea = stateStyle.areaStyle.OpenNormalArea;
                    isOpenCoeifficient = stateStyle.areaStyle.OpenAreaCoefficient;
                    areaDisCoefficient = stateStyle.areaStyle.AreaDisCoefficient;
                    coefficient = stateStyle.areaStyle.AreaCoefficient;
                    isOpenShadow = stateStyle.areaStyle.OpenAreaShadow;
                    shadowDisCoeifficient = stateStyle.areaStyle.AreaShadowDisCoefficient;
                    shadowCoeifficient = stateStyle.areaStyle.AreaShadowCoefficient;
                    GetColor(ref color, stateStyle.areaStyle.color, stateStyle.itemStyle.color, stateStyle.areaStyle.opacity, theme, index);
                    GetColor(ref color, stateStyle.areaStyle.toColor, color, stateStyle.areaStyle.opacity, theme, index);
                    GetColor(ref shadowColor, stateStyle.areaStyle.AreaShadowColor, stateStyle.itemStyle.color, stateStyle.areaStyle.AreaShadowOpacity, theme, index);
                    GetColor(ref shadowToColor, stateStyle.areaStyle.AreaShadowToColor, shadowColor, stateStyle.areaStyle.AreaShadowOpacity, theme, index);
                }
                else
                {
                    return false;
