...
namespace XCharts.Runtime
{
    ...
    internal sealed partial class BarHandler : SerieHandler<Bar>
    {
        ...
        private GameObject _serieItemRoot;
        private List<GameObject> _barItems = new List<GameObject>();
        ...
        {
            UpdateBarSerie(serie);
        }

        public override void DrawSerie(VertexHelper vh)
        {
            if (!serie.itemStyle.Image.show)
            {
        ...
        private void DrawBarSerie(VertexHelper vh, Bar serie)
        {
            ...
            int maxCount = serie.maxShow > 0 ? (serie.maxShow > showData.Count ? showData.Count : serie.maxShow) : showData.Count;
            ...
            _serieItemRoot = ChartHelper.AddObject("items", m_SerieRoot.transform,
            ...
            for (int i = serie.minShow; i < maxCount; i++)
            {
                ...
                serieData.context.backgroundRect =
                    isY ? Rect.MinMaxRect(m_SerieGrid.context.x, plb.y, m_SerieGrid.context.x + relativedAxisLength, prt.y) : Rect.MinMaxRect(plb.x, m_SerieGrid.context.y, prb.x, m_SerieGrid.context.y + relativedAxisLength);
                    ...
                    switch (serie.barType)
                    {
                        ...
                        case BarType.Capsule:
                            DrawNormalBar(vh, serie, serieData, itemStyle, backgroundColor, gap, barWidth,
                                pX, pY, plb, plt, prt, prb, isY, m_SerieGrid, axis, areaColor, areaToColor, relativedValue, i);
                            break;
                        case BarType.Zebra:
        ...
        private void UpdateBarSerie(Bar serie)
        {
            if (serie.barType != BarType.Normal) return;
            ...
            for (int i = 0; i < _serieItemRoot.transform.childCount; i++)
            {
                _serieItemRoot.transform.GetChild(i).localScale = Vector3.zero;
            }
            ...
            for (int i = serie.minShow; i < maxCount; i++)
            {
                ...
                if (serie.show && !serie.placeHolder)
                {
                    if (serie.barType == BarType.Normal)
                    {
                        UpdataNormalBar(serie, serieData, i);
                    }
                }
        ...
        private void UpdataNormalBar(Serie serie, SerieData serieData, int index)
        {
            if (serie.itemStyle.Image.show)
            {
                var item = ChartHelper.AddImageItem("item_" + index, _serieItemRoot.transform,
                    chart.chartMinAnchor, chart.chartMaxAnchor, chart.chartPivot, serieData.data[1], serieData.context.rect.size, serieData.context.rect.center, serie.itemStyle.Image);
            }
        }
        ...
        private void DrawNormalBar(VertexHelper vh, Serie serie, SerieData serieData, ItemStyle itemStyle, Color32 backgroundColor,
            ...
            Vector3 prb, bool isYAxis, GridCoord grid, Axis axis, Color32 areaColor, Color32 areaToColor, double value, int index)
        {
            var borderWidth = itemStyle.runtimeBorderWidth;
            var cornerRadius = serie.barType == BarType.Capsule && !itemStyle.IsNeedCorner() ? m_CapusleDefaultCornerRadius : itemStyle.cornerRadius;
            var invert = value < 0;
            ...
            {
                UGL.DrawBorder(vh, serieData.context.rect, borderWidth, itemStyle.borderColor, itemStyle.borderToColor,
