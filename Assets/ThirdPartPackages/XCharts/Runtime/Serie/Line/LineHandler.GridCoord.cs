    ...
    internal sealed partial class LineHandler : SerieHandler<Line>
    {
        ...
        private void DrawLinePoint(VertexHelper vh, Serie serie)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                //Debug.Log($"<color=blue>Êý¾Ý×ø±ê: {serie.context.dataPoints[i]}  tickness:  {symbolBorder} </color>");
                chart.DrawClipSymbol(vh, symbol.type, symbolSize, symbolBorder, pos,
                    ...
                    symbol.tcOutLineRad,symbol.tcInsideRad,symbol.tcCellRad,symbol.tcOutLineColor,symbol.tcInsideColor,symbol.tcCellColor,
                    symbol.tcOffset,symbol.tcOffsetColor,symbol.tcSAngle,symbol.tcEAngle);
            }
            if (interacting)
            {
                if (SeriesHelper.IsStack(chart.series))
