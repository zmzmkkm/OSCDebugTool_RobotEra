...
namespace XCharts.Runtime
{
    ...
    public class Vessel : MainComponent, ISerieContainer, IUpdateRuntimeData
    {
        public enum Shape
        {
            /// <summary>
        ...
        [SerializeField] private Color32 m_toColor = new Color32(70, 70, 240, 255);
        ...
        [SerializeField] private Color32 m_BorderColor = new Color32(70, 70, 240, 255);
        ...
        /// <summary>
        /// Vessel toColor. The default is consistent with Serie.
        /// ||容器颜色。默认和serie一致。
        /// </summary>
        ...
        {
            if (m_Center.Length < 2) return;
            var chartX = chart.chartX;
            var chartY = chart.chartY;
            var chartWidth = chart.chartWidth;
            var chartHeight = chart.chartHeight;
            var centerX = m_Center[0] <= 1 ? chartWidth * m_Center[0] : m_Center[0];
            var centerY = m_Center[1] <= 1 ? chartHeight * m_Center[1] : m_Center[1];
            var checkWidth = Mathf.Min(chartWidth, chartHeight);
            context.center = new Vector3(chartX + centerX, chartY + centerY);
            context.radius = ChartHelper.GetActualValue(m_Radius, checkWidth);
            context.innerRadius = context.radius - m_ShapeWidth - gap;
            context.width = ChartHelper.GetActualValue(m_Width, checkWidth) - 2 * gap;
            context.height = ChartHelper.GetActualValue(m_Height, chartHeight) - 2 * gap;
        }
    }
}