...
namespace Prospect
{
    public class BarChartHorizontalCtrl : GraphAxisBase
    {
        /// <summary>
        /// 组名，组索引，单柱索引，TipTrs
        /// </summary>
        public Action<string, int, int, Transform> setTipAction;

        /// <summary>
        /// 组名，组索引，单柱索引，背景图片，顶部图片，底部图片，数值文本
        /// </summary>
        public Action<string, int, int, Image, Image, Image, TMP_Text> setBarAction;


        /// <summary>
        /// Y轴数值，组索引，单柱索引，TipTrs
        /// </summary>
        public Action<float, int, int, Transform> setTipActionValue;

        /// <summary>
        /// Y轴数值，组索引，单柱索引，背景图片，顶部图片，底部图片，数值文本
        /// </summary>
        ...
        {
            yAxis0.type = Axis.AxisType.Category;
            ...
            var positives0 = (from item in dataCategory from itemValue in item.values where itemValue.value >= 0 select itemValue.value).ToList();
            if (positives0.Count > 0)
            {
                var max0 = MaxDivisibleValue.GetMaxDivisibleValue(positives0.Max());
                xAxisAtb.axisMax = max0;
            }

            var negatives0 = (from item in dataCategory from itemValue in item.values where itemValue.value < 0 select itemValue.value).ToList();
            if (negatives0.Count > 0)
            {
                var min0 = -MaxDivisibleValue.GetMaxDivisibleValue(Mathf.Abs(negatives0.Min()));
                xAxisAtb.axisMin = (float)min0;
            }
            else
            {
            ...
            {
                yAxisAtb0.axisMax = dataCategory.Count;
                yAxisAtb0.axisMin = 0;
            }
            ...
            for (var i = 0; i < dataCategory.Count; i++)
            {
                ...
                var verticalLayoutGroup = goGroup.gameObject.GetOrAddComponent<VerticalLayoutGroup>();
                ...
                goGroupRec.pivot = new Vector2(0, 0.5f);
                goGroupRec.localScale = Vector3.one;
                goGroupRec.sizeDelta = new Vector2(chartSize.x, goGroupRec.sizeDelta.y);
                var goGroupPos = PointPos(new Vector2(0, i + 0.5f), dataCategory.Count, (float)xAxisAtb.axisMax, 0, xAxisAtb.axisMin) + Vector3.right * offset;
                ...
                for (int j = 0; j < item.values.Count; j++)
                {
                    ...
                    switch (gradualChangeMode)
                    {
                        case GradualChangeMode.单独渐变:
                            xLenght = ValueLenghtX(value.value, (float)xAxisAtb.axisMax, xAxisAtb.axisMin);
                            xLenght = xLenght > 0 ? xLenght : 0;
                        ...
                        case GradualChangeMode.全局渐变:
                            xLenght = BlockSize(value.value > 0);
                            slider.value = 0;
                            yuePercent = ValuePercent(value.value, (float)xAxisAtb.axisMax, xAxisAtb.axisMin);
                    ...
                    //设置样式
                    goChildRect.sizeDelta = new Vector2(xLenght, barWidth);
                    goChildRect.localEulerAngles = value.value < 0 ? new Vector3(180, 0, 180) : Vector3.zero;
                    barBody.sizeDelta = new Vector2(xLenght, barBody.sizeDelta.y);
                    ...
                    switch (barChartType)
                    {
                        case BarChartType.样式_3D:
                            barTopRect.sizeDelta = new Vector2(barTopHeight, barTopRect.sizeDelta.y);
                            barTopImage.enabled = true;
                            barTopImage.sprite = barTopSprite;
                            barTopImage.color = barTopColor;


                            barBottomRect.sizeDelta = new Vector2(barBottomHeight, barBottomRect.sizeDelta.y);
                    ...
                    if (isShowText)
                    {
                        ...
                        if (value.value < 0)
                        {
                            goTextRect.localEulerAngles = new Vector3(180, 0, 180);
                            switch (barTextPos)
                            {
                                case BarTextPos.样式1:
                                    goTextRect.localPosition = fontOffset + Vector2.right * goTextRect.sizeDelta.x;
                                    break;
                                case BarTextPos.样式2:
                                    switch (gradualChangeMode)
                                    {
                                        case GradualChangeMode.单独渐变:
                                            goTextRect.localPosition = -fontOffset - Vector2.right * xLenght;
                                            break;
                                        case GradualChangeMode.全局渐变:
                                            goTextRect.localPosition = -fontOffset - Vector2.right * (yuePercent * xLenght);
                                            break;
                                    }
                                ...
                                case BarTextPos.样式3:
                                    ...
                                    {
                                        case GradualChangeMode.单独渐变:
                                            goTextRect.localPosition = fontOffset + Vector2.right * (BlockSize(false) - xLenght + goTextRect.sizeDelta.x);
                                            break;
                                        case GradualChangeMode.全局渐变:
                                            goTextRect.localPosition = fontOffset + Vector2.right * ((1 - yuePercent) * xLenght + goTextRect.sizeDelta.x);
                                            break;
                                    }
                        ...
                        else
                        {
                            switch (barTextPos)
                            {
                                ...
                                case BarTextPos.样式3:
                                    ...
                                    {
                                        case GradualChangeMode.单独渐变:
                                            goTextRect.localPosition = fontOffset + Vector2.right * (BlockSize(true) - xLenght);
                                            break;
                                        case GradualChangeMode.全局渐变:
                                            goTextRect.localPosition = fontOffset + Vector2.right * (1 - yuePercent) * xLenght;
                                            break;
                                    }
                    ...
                    setBarAction?.Invoke(item.key, i, j, barBodyImage, barTopImage, barBottomImage, goText);
                    ...
                    if (isShowTip)
                    {
                        barBodyImage.OnPointerEnterAsObservable().Subscribe(_ =>
                        {
                            ...
                            setTipAction?.Invoke(item.key, i, j, tip);
                        });
        ...
        {
            yAxis0.type = isTime ? Axis.AxisType.ValueTime : Axis.AxisType.Value;
            yAxis0.minMaxType = Axis.AxisMinMaxType.Custom;
            ...
            if (xAxisAtb.isAutomatic)
            {
                var positives0 = (from item in dataValue from itemValue in item.values where itemValue.value >= 0 select itemValue.value).ToList();
                if (positives0.Count > 0)
                {
                    xAxisAtb.axisMax = MaxDivisibleValue.GetMaxDivisibleValue(positives0.Max());
                }

                var negatives0 = (from item in dataValue from itemValue in item.values where itemValue.value < 0 select itemValue.value).ToList();
                if (negatives0.Count > 0)
                {
                    var min = -MaxDivisibleValue.GetMaxDivisibleValue(Mathf.Abs(negatives0.Min()));
                    xAxisAtb.axisMin = -(float)MaxDivisibleValue.GetMaxDivisibleValue(Mathf.Abs(negatives0.Min()));
                }
                else
                {
                    if (xAxisAtb.axisMin < 0)
            ...
            {
                var positivesY = (from item in dataValue where item.key >= 0 select item.key).ToList();
                if (positivesY.Count > 0)
                {
                    yAxisAtb0.axisMax = MaxDivisibleValue.GetMaxDivisibleValue(positivesY.Max());
                }

                var negativesY = (from item in dataValue where item.key < 0 select item.key).ToList();
            ...
            for (var i = 0; i < dataValue.Count; i++)
            {
                ...
                goGroupRec.localScale = Vector3.one;
                goGroupRec.sizeDelta = new Vector2(chartSize.x, goGroupRec.sizeDelta.y);
                var goGroupPos = PointPos(new Vector2(0, item.key), (float)yAxisAtb0.axisMax, (float)xAxisAtb.axisMax, yAxisAtb0.axisMin, xAxisAtb.axisMin) + Vector3.right * offset;
                goGroup.localPosition = new Vector3(goGroupPos.x, goGroupPos.y, 0);
        ...
        private float BlockSize(bool isPositive)
        {
            ...
            if (xAxisAtb.axisMin >= 0)
            {
                sizeWidth = chartSize.x;
            }
            else
            {
                if (isPositive)
                {
                    sizeWidth = chartSize.x * Mathf.Abs((float)xAxisAtb.axisMax) / ((float)xAxisAtb.axisMax - xAxisAtb.axisMin);
                }
                else
                {
                    sizeWidth = chartSize.x * Mathf.Abs(xAxisAtb.axisMin) / ((float)xAxisAtb.axisMax - xAxisAtb.axisMin);
                }
            }
